...
abstract class BP_Reply_By_Email_Extension {
	...
	public function extend_activity_listener( $listener, $item ) {
		if ( ! empty( $this->data['activity_type'] ) && $item->type == $this->data['activity_type'] ) {
			$listener->component = $this->data['id'];
			$listener->item_id   = $item->item_id;

			if ( ! empty( $this->data['secondary_item_id_param'] ) )
	...
	public function extend_querystring( $querystring, $listener ) {
		...
		if ( $listener->component == $this->data['id'] ) {
			$querystring = "{$this->data['item_id_param']}={$listener->item_id}";

			// some querystrings only use one parameter; if a second one exists,
			// add it.
			if ( ! empty( $this->data['secondary_item_id_param'] ) )
				$querystring .= "&{$this->data['secondary_item_id_param']}={$listener->secondary_item_id}";
		}
	...
	public function register_params( $params ) {
		if ( ! empty( $params[$this->data['item_id_param']] ) ) {
		...
		if ( ! empty( $this->data['secondary_item_id_param'] ) && ! empty( $params[$this->data['secondary_item_id_param']] ) ) {
		...
		$params[$this->data['item_id_param']] = false;

		if ( ! empty( $this->data['secondary_item_id_param'] ) )
			$params[$this->data['secondary_item_id_param']] = false;
