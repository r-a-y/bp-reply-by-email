...
class PluginUpdateChecker_1_3_1 {
	...
	protected function installHooks(){
		...
		add_filter( 'http_request_args', array( $this, 'sslVerify' ), 10, 2 );

		// for github urls, rename directory
		add_filter( 'upgrader_source_selection', array( $this, 'renameGithubDirectory' ), 10, 2 );
	 ...
	 *
	 * Check if download request is from Github.  If so, set the 'sslverify' flag
	 * to false to allow the download to bypass SSL certificate verification.
	 *
	 * @param mixed $args
	 * @param string $url
	 * @return mixed
	 */
	public function sslVerify( $args, $url ) {
	...
	/**
	 * Github .zip packages appends the branch name to the plugin's folder name.
	 *
	 * This isn't desirable. So we check to see if our plugin is hosted on Github.
	 * If so, we remove the branch name from the plugin's folder name to resemble
	 * WP convention.
	 *
	 * @param string $source Temporary location of the extracted folder. Contains
	 *   the plugin folder name with the Github branch appended.
	 * @param string $remote_source Temporary parent location. One level up from
	 *   the extracted folder.
	 * @return string Location of the extracted folder
	 */
	public function renameGithubDirectory( $source, $remote_source ) {
		// if the current plugin is not our plugin, stop now!
		if ( strpos( $source, $this->slug ) === false ) {
			return $source;
		}

		// get our plugin's info from parsed JSON file
		$download_url = ! empty( $this->getUpdateState()->update->download_url ) ? $this->getUpdateState()->update->download_url : false;

		// link isn't from Github, so stop!
		if ( empty( $download_url ) ) {
			return $source;
		} elseif ( strpos( $download_url, 'github.com' ) === false ) {
			return $source;
		}
		...
		// new location
		$new_location = $remote_source . "/{$this->slug}/";

		// rename our github directory
